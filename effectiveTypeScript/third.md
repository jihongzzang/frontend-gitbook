...
- 이를 타입스크립트에서는 인덱스 시그니처를 명시하여 유연하게 매핑하자.
...
인덱스 시그니쳐는 부정확하므로 더 나은 방법을 찾아야함
...
//인덱스 시그니처는 동적 데이터를 표현하고 싶을때 사용함
```

```ts
function parseCSV(input: string): { [columnName: string]: string }[] {
  const lines = input.split('\n');
  const [hader, ...rows] = lines;
...
일반적인 상황에서 열 이름이 무엇인지 미리 알 방법은 없다. 이럴때 인덱스 시그니처를 사용하자. 반면에 미리 열 이름을 알고 있는 특정한 상황에서 CSV가 사용된다면, 미리 선언해 둔 타입으로 단언문을 사용하자
...
물론 런타임에 실제로 일치한다는 보장은 없음. 그래서 undefined를 값 타입에 추가할 수 있음
...
for (const row of safeRows) {
  prices[row.productId] = Number(row.price);
  // undefined를 인덱스 형식에 사용할 수 없다.
}
```

- 어떤 타입에 가능한 필드가 제한되어 있는 경우 인덱스 시그니처로 모델링 하지 말아야한다.

- 예를 들어 데이터가 얼마나 많이 있는지 모를때는 선택적 필드 또는 유니온 타입으로 모델링 하자
...
type Row3 =
  ...
  | { a: number; b: number; c: number; d: number }; // 사용하기에 번거러움
```

- string 타입이 너무 광범위해서 인덱스 시그니처를 사용하는데 문제가 있을때

- Record를 사용해보자 Record는 키타입에 유연성을 제공하는 제네릭 타입
...
- 매핑 된 타입을 사용하자. 매핑왼 타입은 키마다 별도의 타입을 사용하게 해준다.
...
- 안전한 접근을 위해 인덱스 시그니처 값 타입에 undefined를 추가하자

- 가능하다면 인터페이스, Record, 매핑된 타입 같은 인덱스 시그니처보다 정확한 타입을 사용하자
